...
echo       ~ [path]      - cd to C:\[path]$T^
echo       ~u [path]     - cd to C:\Users\USER\[path]$T^
echo       .. [n]        - cd to [n]th super directory shortcut$T^
echo       snaphelp      - Displays this help dialog$T^
@echo on

if not exist C:\snap-plugins mkdir C:\snap-plugins

for %%v in ("C:\snap-plugins\*") do call "%%~v"
